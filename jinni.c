...
enum print_parent_t {PRINT_PARENT_NONE, PRINT_PARENT_CELL_LEFT, PRINT_PARENT_CELL_RIGHT};
void __print_s_expr(S_Expr * se, enum print_parent_t pr_parent);

// evaluation functions:
//S_Expr * apply(S_Expr * fn, S_Expr * args, S_Expr * symbol_table);
S_Expr * apply(S_Expr * fn, S_Expr * args, S_Expr * symbol_table, bool maybe_from_list_constructor, bool args_were_list);
...
{
  if (s >= 1024)
  	printf("\n\ncalled free_mem_segment. size before freeing %d\n\n", s);
  //fflush(stdout);
...
{
  //printf("called free_mem_segment %d\n", s);
  //fflush(stdout);
...
{
  //void * p = (void *)(&system_memory[stack_pointer]);
  //stack_pointer += size;
  //return p;
  //return malloc(size);
  /*
  printf("jinni_malloc -- size: %d, mem_segment_i: %u. mem_segments_ixs[mem_segment_i]: %u\n",
  ...
  */
  ...
  void * p =
    (void *)
    (&mem_segments[mem_segment_i][mem_segments_ixs[mem_segment_i]]);
...
void * jinni_malloc_special(size_t size, char const * caller_name)
{
    printf("jinni_malloc was called from %s\n", caller_name);
    return jinni_malloc(size);
}

//#define jinni_malloc(x)	jinni_malloc_special(x, __func__)
...
bool is_atom(S_Expr * e)
{
	return (e->type == ATOM);
}
/*
void free_list(S_Expr * l)
{
	if (!l) // <-- why?
		return;
	...
	if (l->type == ATOM)
	{
		...
		{
		case STRING:
			jinni_free((char *)a->apval);
			break;
		case NUMBER:
			jinni_free((double *)a->apval);
			break;
		case BOOLEAN:
			jinni_free((bool *)a->apval);
			break;
		case NIL:
			; // nothing to do here, pointer should be null
			break;
		default:
			printf("warning: not freeing, unknown type");
			return;
		}
		jinni_free(a);
	}
	...
	{
		free_list(((Cell *)l->expr)->left);
		free_list(((Cell *)l->expr)->right);
	}
	else
	{
...
bool equal_internal(S_Expr * e1, S_Expr * e2)
{
	...
	case CELL:
		...
		return (equal_internal(c1->left, c2->left) && equal_internal(c1->right, c2->right));
...
void __print_s_expr(S_Expr * se, enum print_parent_t pr_parent)
{
	...
	else if (se->type == CELL)
	{
		...
		if (	pr_parent == PRINT_PARENT_NONE
			|| 	pr_parent == PRINT_PARENT_CELL_LEFT)
		    printf("(");
...
S_Expr * atom(S_Expr * e)
{
	return make_bool_atom(is_atom(e));
}
...
S_Expr * minus(S_Expr * numbers_l)
{
	...
	return make_number_atom(minus_internal(*((double *)a->apval), cdr(numbers_l)));
}
...
S_Expr * multiply(S_Expr * numbers_l)
{
	...
	return make_number_atom(multiply_internal(*((double *)a->apval), cdr(numbers_l)));
}
...
S_Expr * divide(S_Expr * numbers_l)
{
	...
	return make_number_atom(divide_internal(*((double *)a->apval), cdr(numbers_l)));
}
		...
		S_Expr * last)
{
	...
	if (!is_atom(c->left))
	{
		...
		if (is_atom(symbol->left))
		{
			...
			if (label->type == STRING &&
					0 == strcmp((char *)label->apval, pname))
			{
				last = symbol->right;
	...
	{
		printf("warning: left side of symbol table element is not a cell... ignoring it");
	}

	return lookup_symbol_table(pname, cdr(symbol_table), last);
}

S_Expr * bind(S_Expr * st,
		S_Expr * names,
		S_Expr * values,
		uint16_t len)
{
	...
	S_Expr * new_st = append(st, cons(cons(make_string_atom(a_sym_label->pname, strlen(a_sym_label->pname)), car(values)), &nil_s_expr));
		...
		uint16_t len)
{
	...
	Atom * a_sym_label = (Atom *)(car(names)->expr);
	S_Expr * new_st = append(st, cons(cons(make_string_atom(a_sym_label->pname, strlen(a_sym_label->pname)), cons(quote_fn_symbol ,cons(car(values), &nil_s_expr))), &nil_s_expr));

	return bind_quoting(new_st,
			cdr(names),
			cdr(values),
			len);
}
...
S_Expr * apply(S_Expr * fn, S_Expr * args, S_Expr * symbol_table, bool maybe_from_list_constructor, bool args_were_list)
{
	...
	if (is_atom(fn))
	{
		...
		if (!fn_atom->pname) {
			if (fn_atom->type == STRING ||
					fn_atom->type == NUMBER ||
					fn_atom->type == BOOLEAN ||
					fn_atom->type == NIL)
			{
				if (is_nil_atom(args)) {
				...
				jinni_exception("trying to eval non-function symbol with args in apply");
			}
		}
		...
		if (e_atom->len != CALL_ARGUMENTS_LIMIT) {
			...
			if (list_len(args) == 1 &&
				...
				//#ifdef PRINT_DBG_INFO
				printf("expected arity (%d) differs from number of arguments (%d) \n",
						e_atom->len, list_len(args));
				printf("fn_atom pname: %s\n", fn_atom->pname);
				printf("atom pname: %s\n", e_atom->pname);
				printf("e: ");
		...
		*/
		...
		case EXPR:
			...
			S_Expr * new_symbol_table = bind(symbol_table, // not necessary to copy the symbol table! (see other comment on new_symbol_table)
					e_atom->args_bind_names,
					args,
					e_atom->len);

			// values are bound already, the expr defined in the lambda doesn't need any parameters
			// as it takes everything it needs from bound variables, then we call eval on the
			// expr with the new symbol table
	...
	else
	{
			...
			if (fn) {
				...
				if (fn_atom->type == SUBR) {
					...
					if (equal_internal(fn, quote_fn_symbol))
					{
						...
						if (list_len(args) != 1) {
							jinni_exception("expected arity (1) differs from number of args %d\n",
									list_len(args));
						}
						r = quote(car(args));
						goto end_eval;
						...
						r = label(symbol_table, car(args), eval(car(cdr(args)), symbol_table));
					...
					if (fn_atom->args_bind_names)
					{
						...
						new_symbol_table = bind_quoting(*symbol_table,
								fn_atom->args_bind_names,
								args,
								fn_atom->len);
					}
		...
		if (is_atom(car(car(cdr(e))))) {
			...
			if (check_a->type == EXPR) {
				...
				//if (check_a->pname
				//	&& (0 == strcmp(check_a->pname, "CONS") || 0 == strcmp(check_a->pname, "QUOTE")))
				//	maybe_from_list_constructor = true;
			}
		}

		bool args_were_list = (false == is_atom(cdr(e)) && false == is_atom(cdr(cdr(e))));

		print_debug_info("eval - cdr(e):", cdr(e));
		S_Expr * args = evlis(cdr(e), *symbol_table);
		print_debug_info("args:", args);

		r = apply(car(e), args, *symbol_table, maybe_from_list_constructor, args_were_list);
			...
			if (l != NULL) {
				...
				if (l_a->len > 0) {
					S_Expr * e2 = eval(e_cared, &symbol_table);
					{
						...
						//char dbg_msg[256];
						//sprintf(dbg_msg, "evlis -- function %s evaluated to: ", head_a->pname);
						//print_debug_info(dbg_msg, e2);
...
// Making our own little 'atoi' here, since, should
// we wanted to build with no stdlib, it'd come in handy :)
//
// ... ok, that comment ^^ is seriously outdated!
S_Expr * parse_number(char * number_str, int * consumed)
{
	if (*number_str == '\0')
		return NULL;
...
#define SYMBOL_NAME_MAX_LEN 256
	...
	*consumed = p-symbol_str;
	...
	((Atom *)(se->expr))->pname = jinni_malloc(sizeof(char) * (strlen(symbol_name)+1));
